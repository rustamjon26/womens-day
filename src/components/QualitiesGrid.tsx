...
const QualitiesGrid: React.FC = () => {
  return (
    <div className="w-full max-w-7xl mx-auto px-4 py-24">
      ...
      >
        <h2 className="text-5xl md:text-6xl font-serif mb-8 text-primary font-bold">
          Nega Siz Maxsus?
        </h2>
        <p className="text-muted-foreground max-w-3xl mx-auto text-xl leading-relaxed font-medium px-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
        {qualities.map((item, i) => (
          <motion.div
            ...
            className={`glass p-10 rounded-[2.5rem] border-2 ${item.color} group relative overflow-hidden shadow-[0_20px_40px_-15px_rgba(0,0,0,0.05)] hover:shadow-[0_40px_80px_-20px_rgba(225,29,72,0.1)] transition-all duration-500`}
          >
            ...
            <div className="mb-8 p-5 rounded-2xl bg-white w-fit shadow-xl shadow-black/5 group-hover:scale-110 transition-transform duration-500">
              {item.icon}
            </div>

            <h3 className="text-3xl font-serif mb-5 text-primary font-bold">
              {item.title}
            </h3>
